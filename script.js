...
function openEditMode() {
    ...
    currentImageBase64 = currentRecipe.img || null; 
...
function resetForm() { 
    ...
    document.getElementById('add-photo-preview-container').innerHTML = `<i class="fas fa-camera text-2xl mb-2"></i><span class="text-xs font-bold">Ajouter une photo</span>`; 
}
...
function checkSeasonality(i) { 
    if(!i) return false; 
    const m = new Date().getMonth(), w = (m >= 10 || m <= 2); 
    if (w) { return i.some(x => ['tomate', 'courgette', 'aubergine', 'fraise', 'melon', 'pêche', 'concombre', 'poivron'].some(b => x.toLowerCase().includes(b))); } 
    return false; 
}

function changePortion(d) { 
    if(currentPortion + d < 0.5) return; 
    currentPortion += d; 
    document.getElementById('portion-count').textContent = currentPortion; 
    renderIngredientsList(); 
}

function getScaledIngredients(i, p) { 
    const r = p; 
    return i.map(l => { 
        const m = l.match(/^(\d+(?:[\.,]\d+)?)\s*(.*)$/); 
        if (m && p !== 1) { 
            return `${Math.round((parseFloat(m[1].replace(',', '.')) * r) * 10) / 10} ${m[2]}`; 
        } 
        return l; 
    }); 
}
...
function openBadges() { 
    ...
    getBadgesList().forEach(b => { 
        const u = b.cond(userStats); 
        l.innerHTML += `<div onclick="alert('${b.title} : ${b.desc}')" class="flex flex-col items-center justify-center p-3 rounded-2xl bg-gray-50 border ${u ? 'border-purple-200 bg-purple-50' : 'border-gray-100'} ${u ? '' : 'badge-locked'} cursor-pointer"><div class="text-3xl mb-1">${b.icon}</div><div class="text-[10px] font-bold text-center leading-tight ${u ? 'text-purple-700' : 'text-gray-400'}">${b.title}</div></div>`; 
    }); 
...
function fetchRecipeFromUrl() { 
    const u = document.getElementById('import-url').value; 
    const b = document.getElementById('btn-import-action'); 
    if(!u) return; 
    const t = b.innerHTML; 
    b.innerHTML = '...'; b.disabled = true; 
    const p = 'https://corsproxy.io/?' + encodeURIComponent(u); 
    fetch(p).then(r => { if(!r.ok) throw new Error("Err"); return r.text(); })
    .then(h => { 
...
function addFridgeItem() { 
    const i = document.getElementById('fridge-input'); 
    const v = i.value.trim(); 
    if(v && !fridgeIngredients.includes(v)) { 
        fridgeIngredients.push(v); i.value = ""; renderFridgeTags(); 
    } 
}
...
function renderFridgeTags() { 
    const d = document.getElementById('fridge-tags'); d.innerHTML = ""; 
    fridgeIngredients.forEach(i => { 
        d.innerHTML += `<button onclick="removeFridgeItem('${i}')" class="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm font-bold flex items-center gap-2">${i} <i class="fas fa-times"></i></button>`; 
    }); 
}

function searchFridge() { 
    const d = document.getElementById('fridge-results'); d.innerHTML = ""; 
    if(fridgeIngredients.length === 0) { d.innerHTML = `<div class="text-center text-gray-400">Ajoute des ingrédients !</div>`; return; } 
    const m = allRecipes.filter(r => r.i.some(ri => fridgeIngredients.some(fw => ri.toLowerCase().includes(fw.toLowerCase())))); 
    if(m.length === 0) { d.innerHTML = `<div class="text-center text-gray-400">Rien trouvé...</div>`; } 
    else { renderCookbookList(m, 'fridge-results'); } 
}

function shareRecipe() { 
    if(!currentRecipe) return; 
    const t = `👨‍🍳 Je cuisine : ${currentRecipe.t} (${currentRecipe.time}min).\n${currentRecipe.i.join(', ')}.`; 
    if(navigator.share) { navigator.share({title: 'FoodMood', text: t}); } 
