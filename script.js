...
// ID Unique pour synchroniser les données de CET utilisateur
// (On garde le localStorage juste pour l'ID, afin que l'utilisateur retrouve SES données)
const USER_ID = localStorage.getItem('foodmood_uid') || 'user_' + Date.now();
localStorage.setItem('foodmood_uid', USER_ID);
...
// --- INIT WITH FIREBASE LISTENER ---
document.addEventListener('DOMContentLoaded', () => { 
    initFirebaseListener();
});

function initFirebaseListener() {
    // Écoute en temps réel les changements sur la base de données
    onValue(userRef, (snapshot) => {
        ...
        // Reconstruction de allRecipes (Base + Externe + Perso)
        loadRecipes(userLocalRecipes).then(() => {
            // Mise à jour de toute l'interface
            updateStatsUI();
            renderShoppingList();
            renderSettingsTags();
            // Si on est dans le livre de cuisine, on rafraichit
...
async function loadRecipes(userLocalRecipes) {
    ...
    // RECIPES_DATA vient de recettes.js (global)
...
// --- HELPER : UPDATE FIREBASE ---
function syncToFirebase(path, data) {
    // path ex: 'recipes', 'fav', 'shop', 'tags', 'stats'
    set(ref(db, `users/${USER_ID}/${path}`), data);
}
...
function addNewTag() {
    ...
    if(val && !userTags.includes(val)) {
        userTags.push(val);
        syncToFirebase('tags', userTags); // Sync
        input.value = "";
    }
}
...
function renderSettingsTags() {
    ...
    div.innerHTML = "";
    userTags.forEach(t => {
        div.innerHTML += `<div class="bg-gray-100 px-3 py-1 rounded-full text-sm flex items-center gap-2">${t} <button onclick="window.removeTag('${t}')" class="text-red-500"><i class="fas fa-times"></i></button></div>`;
    });
}
...
function updateStatsOnClick() {
    ...
    // Mise à jour de l'objet local d'abord
    userStats.total++;
    ...
    syncToFirebase('stats', userStats); // Sync
    updateStatsUI();
}
...
function toggleFavorite() {
    ...
    syncToFirebase('fav', favorites); // Sync
...
function renderFridgeTags() {
    ...
    fridgeIngredients.forEach(i => { div.innerHTML += `<button onclick="window.removeFridgeItem('${i}')" class="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm font-bold flex items-center gap-2">${i} <i class="fas fa-times"></i></button>`; });
}
...
function toggleShopItem(idx) { 
    ...
    syncToFirebase('shop', shoppingList);
}
...
function renderShoppingList() {
    ...
    Object.keys(groups).sort().forEach(cat => {
        ...
        groups[cat].forEach(item => {
            container.innerHTML += `<div onclick="window.toggleShopItem(${item.originalIdx})" class="bg-white p-3 rounded-xl border border-gray-100 flex items-center gap-3 active:scale-[0.98] transition cursor-pointer mb-2"><div class="w-5 h-5 rounded border ${item.done?'bg-brand border-brand':'border-gray-300'} flex items-center justify-center">${item.done?'<i class="fas fa-check text-white text-xs"></i>':''}</div><span class="${item.done?'line-through text-gray-400':'text-gray-800'} font-medium">${item.t}</span></div>`;
        });
    });
}
...
function renderCookbookTagsFilter() {
    ...
    userTags.forEach(t => {
        ...
        div.innerHTML += `<button onclick="window.toggleTagFilter('${t}')" class="tag-select flex-shrink-0 px-3 py-1 rounded-full text-[10px] font-bold border transition ${isActive ? 'active' : 'bg-white border-gray-200 text-gray-500'}">${t}</button>`;
    });
}
...
function filterCookbook() {
    ...
    const filtered = allRecipes.filter(r => {
        ...
        let matchTag = true;
        if(activeTagFilter) { matchTag = r.tags && r.tags.includes(activeTagFilter); }
...
async function fetchRecipeFromUrl() {
    ...
    try {
        ...
        if (recipeData) {
            ...
            userStats.imported++; syncToFirebase('stats', userStats); // Sync
...
function saveRecipe() {
    ...
    // Pour Firebase, on récupère juste la liste actuelle des recettes perso
    // allRecipes contient tout, on doit filtrer pour n'avoir que celles > 1000 ou user created
    // Simplification : on récupère les recettes perso depuis allRecipes
    ...
    syncToFirebase('recipes', userRecipes); // Sync
    currentRecipe = recipeData;
    // La mise à jour UI se fera via le listener onValue
    navigate('result');
...
// Export/Import restent possibles mais c'est moins critique avec Firebase
function exportData() {
    const data = { recipes: allRecipes.filter(r=>r.id>1000), fav: favorites, shop: shoppingList, tags: userTags, stats: userStats };
...
function importData(input) {
    ...
    reader.onload = function(e) {
        ...
        if(data.recipes) syncToFirebase('recipes', data.recipes);
        if(data.fav) syncToFirebase('fav', data.fav);
        if(data.shop) syncToFirebase('shop', data.shop);
        if(data.tags) syncToFirebase('tags', data.tags);
        if(data.stats) syncToFirebase('stats', data.stats);
        alert("Données restaurées et synchronisées !");
    };
    reader.readAsText(file);
}

// EXPORT TO WINDOW (IMPORTANT pour que le HTML fonctionne)
window.navigate = navigate;
window.openBadges = openBadges;
window.toggleSettings = toggleSettings;
window.openAddMode = openAddMode;
window.findRecipe = findRecipe;
window.openCookbook = openCookbook;
window.toggleFavFilter = toggleFavFilter;
window.setCategoryFilter = setCategoryFilter;
window.filterCookbook = filterCookbook;
window.clearShoppingList = clearShoppingList;
window.addShopItemManually = addShopItemManually;
window.addFridgeItem = addFridgeItem;
window.searchFridge = searchFridge;
window.shareRecipe = shareRecipe;
window.toggleFavorite = toggleFavorite;
window.changePortion = changePortion;
window.addAllToShop = addAllToShop;
...
window.updateStatsOnClick = updateStatsOnClick;
window.renderResult = renderResult;
window.allRecipes = allRecipes; // Pour l'accès direct depuis le HTML
